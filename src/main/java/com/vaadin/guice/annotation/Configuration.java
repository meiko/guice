...
public @interface Configuration {
     ...
     * default ( no-args ) constructor. There are three interfaces that will be recognized when
     * implemented by one of the classes in modules:
     *
     * Every class that implements {@link com.vaadin.guice.server.NeedsInjector} will have it's
     * {@link com.vaadin.guice.server.NeedsInjector#setInjectorProvider(Provider)} method called
     * with a provider for the {@link com.google.inject.Injector} that holds the Guice-context. The
     * {@link Provider} will throw a {@link NullPointerException} on {@link Provider#get()} if the
     * {@link com.google.inject.Injector} is not ready yet.
     *
     * Every class that implements {@link com.vaadin.guice.server.NeedsReflections} will have it's
     * {@link com.vaadin.guice.server.NeedsReflections#setReflections(Reflections)} method called
     * with the {@link Reflections} that are being used to set up the Guice-context.
     */
